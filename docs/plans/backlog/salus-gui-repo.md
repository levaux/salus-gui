...
3. **Vendored protos at a pinned commit — not a submodule.** `tools/sync-protos.ts` copies
   ...
   `packages/proto/salus-commit.lock`. Three CI gates: `sync-protos:check` (unintentional drift,
   needs no Salus checkout), committed-codegen diff, and `buf breaking` against a committed
   baseline image (intentional-but-incompatible bumps forced through review). Codegen is
   **buf v2 + protoc-gen-es v2** (`target=ts`, output committed). Record as
   `docs/adr/002-proto-vendoring.md`.
...
6. **Single test tier.** vitest across every package and app, `environment: 'node'`, colocated
   `src/**/*.test.ts`. No browser-automation tier; browser-only behaviour is verified manually
...
(corepack → pnpm cache → install → lint → typecheck; later stages append their gates).
...
`target=ts`, `src/gen` committed); the **well-known choke point** (`well-known.ts`: Timestamp ↔
...
mass-resubscribe). The two reconnect contracts are documented in the package README and pinned
by vitest: **SeqResume** (logs/lifecycle resume by `seq` via the `salus-log-since-seq` metadata
convention) and **Resnapshot** (subscribe-then-snapshot ordering, no lost-update window).
...
static serving of the built SPA in production. vitest: forward guard, header hygiene, KV
round-trip, read-only refusal.
...
- CI green on a fresh checkout (the codegen-before-typecheck rule proven by CI ordering).
...
- **Upstream proto drift discipline.** The lock pins a Salus SHA; a Salus proto change lands
  here only via `pnpm sync-protos` + regenerated codegen in one `proto:` commit, with
  `buf breaking` forcing review of incompatible bumps. Never hand-edit `vendor/` or `src/gen`.
